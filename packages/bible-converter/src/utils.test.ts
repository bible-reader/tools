...
describe("getHash()", () => {
  it("should always return the same thing for the same payload", () => {
    expect(getHash("hello")).toBe("aaf4c61d");
    ...
    expect(getHash("hallo")).toBe("fd4cef7a");
    const obj = {
      x: "x",
      a: 2,
      o: {
    ...
    expect(getHash(JSON.stringify(obj))).toBe("f13feb95");
  });
  it("should return default length of 8", () => {
    expect(getHash("hello").length).toBe(8);
  });
